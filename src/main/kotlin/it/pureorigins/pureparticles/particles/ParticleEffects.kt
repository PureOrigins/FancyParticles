...
object ParticleEffects {
    ...
    val FIRE_SPIRAL = register(
        "fire_spiral",
        OrbitalParticle(angularSpeed = PI / 18) madeOf FLAME period 2 at FEET,
    ...
    val KNOWLEDGE = register(
        "knowledge",
        ParallelepipedParticle(speed = 0.75) madeOf ENCHANTMENT_TABLE at HEAD
    )

    val ASHEN = register(
        "ashen",
        ParallelepipedParticle(0.1, 0.05, 0.1, 5) madeOf SMOKE_LARGE period 3 at HEAD
    )

    val SHINING = register(
        "shining",
        ParallelepipedParticle(0.25, 0.5, 0.25) madeOf VILLAGER_HAPPY period 10 at FEET atY 0.9
    )
    ...
    fun fromId(id: String): ParticleEffect? {
        return effects[id]
    }
}
